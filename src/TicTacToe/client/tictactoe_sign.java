...
import TicTacToe.server.MemberDAO;
import TicTacToe.server.MemberDTO;
...
public class tictactoe_sign extends JFrame implements ActionListener { 
	...
	private TictactoeClientSocket csocket;
	...
	public tictactoe_sign() {
		...
		csocket = new TictactoeClientSocket();
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == idButton) {// id��ư
			...
			String packet = String.format("idcheck|%s", id);
			csocket.sendPacket(packet);
			boolean check = true;
			
			String idcheck = csocket.recivePacket();
		  //String[] strarr = csocket.parsePacket(idcheck);
		...
		} else if (e.getSource() == signCompleteButton) { //회원가입 버튼
			String id = idText.getText();
			...
			String packet = String.format("sign|%s|%s", id, pwd);
			csocket.sendPacket(packet);
			
			String sign = csocket.recivePacket();
			
			//ServerSocketThread에서 회원가입 완료됬다고 enter를 쳐서 받음(신호받음)
